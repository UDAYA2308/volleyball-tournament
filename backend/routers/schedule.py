        ...
        for row in rows:
            if row["match_id"]:
                ...
                row["team_a_sets_won"] = sum(
                    1 for s in row["sets"]
                    if s["winner_team_id"] == row["team_a_id"]
                )
                row["team_b_sets_won"] = sum(
                    1 for s in row["sets"]
                    if s["winner_team_id"] == row["team_b_id"]
                )
            else:
                row["sets"] = []
...
def get_schedule_by_type(match_type: str):
    ...
    if match_type not in valid_types:
        raise HTTPException(status_code=400, detail=f"Invalid match type. Must be one of {valid_types}")

...
def play_set(match_id, team_a_id, team_b_id,
    ...
    while score_a < target_a or score_b < target_b:
        ...
        if not has_server:
            if serving_team == team_a_id:
                ...
                server = random.choice(
                    team_a_players if random.random() > 0.5
                    else team_b_players
                )
        ...
        if need_a == 0:
            ...
            total  = need_a + need_b
            scorer = (
                team_a_id if random.random() < (need_a / total)
                else team_b_id
            )
...
if __name__ == "__main__":
    ...
    if not final["match_id"]:
        print("\n❌ Final has no match row yet — "
              "make sure Qualifier 2 is completed")
        exit(1)
    ...
    champ   = next(
        (m for m in matches
         if m["match_type"] == "final" and m["winner"]),
        None
    )
    if champ:
        print(f"\n{'='*60}")

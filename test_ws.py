    ...
    async with websockets.connect(uri) as ws:
        ...
        while True:
            ...
            if isinstance(match_data, dict):
                print(f"  {match_data.get('team_a_name')} vs {match_data.get('team_b_name')}")
                print(f"  Score:  {match_data.get('team_a_score')}-{match_data.get('team_b_score')}")
                print(f"  Set:    {match_data.get('set_number')}")
                print(f"  Server: {match_data.get('current_server_name')} ({match_data.get('serving_team_name')})")

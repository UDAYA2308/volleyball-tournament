...
export default function Playoffs() {
  ...
  useEffect(() => {
    ...
    load()
    // Poll every 15 seconds to catch bracket updates
    ...
    <div className="space-y-6 max-w-2xl mx-auto">
      ...
      {isLeague && (
        <div className="bg-slate-800 border border-slate-700 rounded-xl p-6
          ...
          <div className="text-white font-bold">League Stage In Progress</div>
          ...
          <div className="text-slate-500 text-xs mt-2">
            Playoffs will be announced once all league matches are done
          </div>
        </div>
      )}
      ...
      {/* Bracket */}
      {(isPlayoffs || isCompleted) && bracket?.bracket?.length > 0 && (
              ...
              <div
                ...
                onClick={() => {
                  if (entry.match_id && !isTBD) {
                    if (isAdmin) {
                      navigate(`/admin/match/${entry.match_id}`)
                    } else {
                  ...
                  <div className={`grid gap-1.5 mt-3
                    ...
                    {entry.sets.map(s => (
                      <div key={s.set_number}
                        className="bg-slate-800/80 rounded-lg px-2 py-1.5
                                   text-center">

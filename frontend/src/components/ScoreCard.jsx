...
export default function ScoreCard({ match, onClick }) {
  ...
  return (
    <div
      onClick={onClick}
      className={`relative rounded-xl border border-theme transition-all
                  duration-200 cursor-pointer overflow-hidden bg-theme-card
        ${isLive ? 'border-red-500/60 hover:border-red-400'
                 : 'hover:border-blue-400/50'}`}
    >
      {/* Top accent bar */}
      <div className={`h-1 w-full
        ${isLive      ? 'bg-gradient-to-r from-red-500 via-orange-400 to-red-500'
        : isCompleted ? 'bg-gradient-to-r from-blue-500 via-blue-400 to-blue-500'
        : isPlayoff   ? 'bg-gradient-to-r from-purple-500 via-purple-400 to-purple-500'
        : 'bg-theme-input'}`}
      />
          ...
          <div className={`flex items-center justify-between gap-2
            ...
            <div className="flex items-center gap-1.5 truncate flex-1
              ...
              <span className={`font-bold text-xs sm:text-sm truncate
                ...
                : isCompleted             ? 'text-theme-secondary'
                :                           'text-theme-primary'}`}>
                {match.team_a_name}
              </span>
            </div>
              ...
              return (
                ...
                >
                  ...
                  <div className="font-mono font-black text-xs sm:text-sm">
                    <span className={setAWon ? 'text-blue-400'
                                             : 'text-theme-primary'}>
                      {setData.team_a_score}
                    </span>
                    <span className="text-theme-secondary mx-0.5">-</span>
                    <span className={setBWon ? 'text-orange-400'
                                             : 'text-theme-primary'}>
